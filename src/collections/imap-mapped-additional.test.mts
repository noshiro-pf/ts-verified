...
describe('IMapMapped additional functionality', () => {
  describe('IMapMapped.create', () => {
    it('should create empty map', () => {
      const map = IMapMapped.create<TestKey, string, string>([], testKeyToString, stringToTestKey);
      expect(map.size).toBe(0);
    });

    it('should create map with initial entries', () => {
      const entries: Array<readonly [TestKey, string]> = [
  ...
  describe('IMapMapped.equal', () => {
    it('should return true for equal maps', () => {
      const entries: Array<readonly [TestKey, string]> = [[{ id: 1, type: 'user' }, 'Alice']];
  ...
  describe('every method', () => {
    it('should return true when all elements satisfy predicate', () => {
      const map = IMapMapped.create<TestKey, number, string>(
        [[{ id: 1, type: 'user' }, 2], [{ id: 2, type: 'admin' }, 4]],
        testKeyToString,
        stringToTestKey
      );
    ...
    it('should return false when some elements do not satisfy predicate', () => {
      const map = IMapMapped.create<TestKey, number, string>(
        [[{ id: 1, type: 'user' }, 2], [{ id: 2, type: 'admin' }, 3]],
        testKeyToString,
        stringToTestKey
      );
    ...
    it('should return true for empty map', () => {
      const map = IMapMapped.create<TestKey, number, string>([], testKeyToString, stringToTestKey);
  ...
  describe('some method', () => {
    it('should return true when at least one element satisfies predicate', () => {
      const map = IMapMapped.create<TestKey, number, string>(
        [[{ id: 1, type: 'user' }, 1], [{ id: 2, type: 'admin' }, 4]],
        testKeyToString,
        stringToTestKey
      );
    ...
    it('should return false when no elements satisfy predicate', () => {
      const map = IMapMapped.create<TestKey, number, string>(
        [[{ id: 1, type: 'user' }, 1], [{ id: 2, type: 'admin' }, 3]],
        testKeyToString,
        stringToTestKey
      );
  ...
  describe('withMutations method', () => {
    it('should apply multiple mutations', () => {
      ...
      const updated = map.withMutations([
        ...
        { type: 'update', key: { id: 1, type: 'user' }, updater: (x: number) => x * 2 },
  ...
  describe('map method', () => {
    it('should transform values', () => {
      const map = IMapMapped.create<TestKey, number, string>(
        [[{ id: 1, type: 'user' }, 5], [{ id: 2, type: 'admin' }, 10]],
        testKeyToString,
        stringToTestKey
      );
    ...
    it('should work with key in mapping function', () => {
      const map = IMapMapped.create<TestKey, string, string>(
        [[{ id: 1, type: 'user' }, 'Alice'], [{ id: 2, type: 'admin' }, 'Bob']],
        testKeyToString,
        stringToTestKey
      );
      const mapped = map.map((value, key) => `${key.type}: ${value}`);
      
      expect(Optional.unwrap(mapped.get({ id: 1, type: 'user' }))).toBe('user: Alice');
      expect(Optional.unwrap(mapped.get({ id: 2, type: 'admin' }))).toBe('admin: Bob');
    });
  });
  ...
  describe('mapKeys method', () => {
    it('should transform keys', () => {
      ...
      expect(Optional.unwrap(mapped.get({ id: 101, type: 'user' }))).toBe('Alice');
    });
  });
