...
describe('Arr', () => {
  describe('zeros', () => {
    ...
    test('negative length', () => {
      const result = Arr.zeros(-1);
      expectType<typeof result, readonly 0[]>('=');
      expect(result).toStrictEqual([]); // Array.from behavior
    });
  });
  ...
  describe('newArray', () => {
    ...
    test('unknown length', () => {
      const n: number = 2;
      const result = Arr.newArray(n, true);
      expectType<typeof result, readonly true[]>('=');
      expect(result).toStrictEqual([true, true]);
    });

    test('negative length', () => {
      const result = Arr.newArray(-1, 'test');
      expectType<typeof result, readonly 'test'[]>('=');
      expect(result).toStrictEqual([]); // Array.from behavior
    });
  });
  ...
  describe('range', () => {
    ...
    test('start < end, step > 1', () => {
      ...
      expectType<typeof result, readonly number[]>('='); // Type is less specific with explicit step
    ...
    test('unknown start/end/step', () => {
      ...
      const end: number = 4;
      const step: number = 1;
      const result = Arr.range(start, end, step);
      expectType<typeof result, readonly number[]>('=');
